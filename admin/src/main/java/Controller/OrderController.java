...
@MultipartConfig
@WebServlet(urlPatterns = {"/admin-order","/admin-category-accept","/admin-category-delete","/admin-category-complete"})
public class OrderController extends HttpServlet {
    ...
    protected void accept(HttpServletRequest request, HttpServletResponse response) throws IOException,ServletException{
        try{
            ...
            order.setOrderStatus("Shipping"); //Is modified
    ...
    protected void complete(HttpServletRequest request, HttpServletResponse response) throws IOException,ServletException{
        try{
            OrderEntity order = orderService.findById(Integer.parseInt(request.getParameter("orderId")));
            order.setOrderStatus("Complete");
            orderService.update(order);
            request.setAttribute("success","Complete Order Successfully");
        }catch (Exception ex)
        {
            ex.printStackTrace();
            request.setAttribute("error","Error: "+ex.getMessage());
    ...
    protected void delete(HttpServletRequest request, HttpServletResponse response) throws IOException,ServletException{
        try{
            //3 rows below are modified
            /*orderService.delete(Integer.parseInt(request.getParameter("orderId")));*/
            OrderEntity order = orderService.findById(Integer.parseInt(request.getParameter("orderId")));
            order.setOrderStatus("Cancel");
            orderService.update(order);
            request.setAttribute("success","Cancel Order Successfully");
        }catch (Exception ex)
        {
            ex.printStackTrace();
            request.setAttribute("error","Error: "+ex.getMessage());
